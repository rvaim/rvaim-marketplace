...
如果用户只要求审查已有插件，优先使用 `review-plugin`。本插件不再生成 hook 配置；需要 hook 的项目应由用户在目标插件中手动实现。
...
2. 同一个插件根目录同时包含：
   ...
   - `skills/`
   - `docs/`
3. 不生成 hooks，manifest 不声明 `hooks` 字段。
4. 默认不生成 `bin/`。
5. 文档默认中文。
6. 插件名、skill 名使用 kebab-case。
7. 大型资料放到 `docs/` 或 `skills/<skill>/references/`，不要塞进 `SKILL.md`。
8. 使用 `templates/dual-plugin/` 时必须替换所有 `{{...}}` 占位符，不要留下模板值。
9. 配置字段尽量完整，但不能伪造不存在的 homepage、repository、license、privacyPolicyURL 或 termsOfServiceURL。
10. 发布面信息优先放在 manifest 和 marketplace：名称、描述、作者、主页、仓库、关键词、分类、能力、默认提示。
...
7. 安装命令。
8. 已替换的模板占位符清单。
9. 如果用户要求 LSP、MCP、monitors、userConfig、dependencies、openai.yaml 或 settings.json，输出对应文件和不默认生成的理由。
10. 验证清单：JSON 是否有效、manifest 路径是否以 `./` 开头、是否存在未替换占位符、版本是否同步。
