...
创建 Agent 时，插件不传 `memory`、`memfs`、`cwd` 或存储资源；由 Letta 使用自己的默认行为。后台 Session 使用 `strict` 权限模式，但不提供自定义 `allowedTools`、`skillSources` 或 `canUseTool`，因此 Letta 保留自己的原生工具与 skills 决策。
...
状态命名空间只由 App Server 地址和能力令牌决定，不再包含 backend 或记忆模式。版本 `2.4.0` 恢复使用原有的每工作区命名空间，因此旧工作区映射和待处理队列可以继续复用。
